...
\title{%
	...
	\begin{algorithm}
		...
		\While{get next argument if not 0}{
			\uIf{argument is a}{ run\_all is true }
			\uElseIf{argument is b, s, q, or Q and run\_all is false}{ append argument to execute\; }
			\uElseIf{argument is r}{ seed = argument value\; }
			\uElseIf{argument is n}{ size = argument value\; }
			\uElseIf{argument is p}{ elements = argument value\; }
			\uElse{ print invalid command\; \Return{Error}
			}
